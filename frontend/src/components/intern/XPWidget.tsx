...
export const XPWidget: React.FC<XPWidgetProps> = ({ totalXP, level, currentStreak }) => {
  ...
  return (
    <div className="relative overflow-hidden bg-white rounded-3xl border border-slate-100 shadow-sm p-5 sm:p-6 mb-6">
      {/* Decorative glows */}
      <div className="absolute top-0 right-0 w-64 h-64 bg-indigo-50/40 rounded-full blur-3xl pointer-events-none -mr-20 -mt-20" />
      <div className="absolute bottom-0 left-0 w-48 h-48 bg-purple-50/30 rounded-full blur-3xl pointer-events-none -ml-16 -mb-16" />
      ...
      <div className="relative z-10 flex flex-col md:flex-row items-stretch md:items-center justify-between gap-5 text-left">
        ...
        <div className="flex items-center gap-4">
          <div className="relative flex-shrink-0">
            <div className="w-14 h-14 bg-gradient-to-br from-indigo-500 to-purple-600 rounded-2xl flex items-center justify-center shadow-md shadow-indigo-500/20 border border-indigo-400/20">
          ...
          <div>
            <div className="flex items-center gap-1.5">
              <h4 className="text-sm font-extrabold text-slate-800 tracking-tight">Level Progress</h4>
        ...
        <div className="flex-grow max-w-md space-y-1.5">
          ...
          <div className="relative w-full h-3 bg-slate-100 rounded-full overflow-hidden border border-slate-200/50">
            <div
              className="absolute top-0 left-0 h-full bg-gradient-to-r from-indigo-500 to-purple-500 rounded-full transition-all duration-500 ease-out shadow-sm"
              style={{ width: `${progressPercent}%` }}
            />
          </div>
          <div className="flex justify-end">
            <span className="text-[10px] font-extrabold text-indigo-600 tracking-wide font-mono">
        ...
        {/* Streak Component */}
        <div className="flex-shrink-0 flex items-center gap-3 bg-slate-50 border border-slate-200/50 px-5 py-3 rounded-2xl">
          <div className={`p-2 rounded-xl flex items-center justify-center ${currentStreak > 0 ? 'bg-amber-500/10 text-amber-500' : 'bg-slate-200/50 text-slate-400'}`}>
